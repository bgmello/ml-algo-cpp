...
double IncrementalClustering::jaccard(std::set<std::string> ngram1, std::set<std::string> ngram2)
{
	if(ngram1.size()==0 or ngram2.size()==0)return 0;

	std::set<std::string> intersect;

	std::set_intersection(ngram1.begin(), ngram1.end(), ngram2.begin(), ngram2.end(), std::inserter(intersect, intersect.begin()));

	return intersect.size()/(ngram1.size()+ngram2.size()-intersect.size());
}

double IncrementalClustering::distance(std::set<std::string> ngram1, std::set<std::string> ngram2)
{
	return 1 - this->jaccard(ngram1, ngram2);
}


std::vector<int> IncrementalClustering::fit_predict(std::vector<std::string> sentences)
{
